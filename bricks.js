...
function makeStar (a, b, points) {
  const result = []

  const alpha = (Math.PI * 2) / points
  for (let i = points; i > 0; i--) {
    const radius = i % 2 ? a : b
    result.push([radius * Math.sin(alpha * i), radius * Math.cos(alpha * i)])
  }
...
function main () {
  ...
  // const star = makeStar(20, 2, 10)
  // console.log('star:', star)
  const star = [
    [-10.755705045849467, 12.180339887498945],
    [-1.9021130325903073, 0.6180339887498945],
    [-14.02113032590307, -2.180339887498951],
    [4.1755705045849465, -1.6180339887498947]
  ]
  ...
  for (let i = 0; i < 1; i++) {
    ...
    result.push(translate([0, 0, h], iterateEdges(star, winding, brickInfo, showMortarSlices)))
  }
...
function iterateEdges (points, winding, brickInfo, showMortarSlices = false) {
  ...
  for (let i = 0; i < cornerCuts.length; i++) {
    ...
    // a, b, c, and d all correspond to points[i + 1], not points[i]
    const ppp = points[i]
    const ccc = (i < points.length - 1) ? points[i + 1] : points[0]
    const nnn = (i < points.length - 2) ? points[i + 2] : points[1]

    // we calculate the angle using points from the original polygon so we don't have to care about
    const angle = getAngle(ppp, ccc, nnn)

    if (angle < 90) {
      result.push(layOnLineEx(b, d, zeroedCuboid(brickInfo.brickHeight, len(b, d), brickInfo.mortarThickness)))
    } else if (angle > 270) {
